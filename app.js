...
const { users, admins, events, clubInfos, participants } = require("./mongo");
	...
	try {
		...
		if (user) {
			...
			if (match) {
				...
				res.status(400).json({ status: "doesnotmatch" });
...
app.post("/enroll", async (req, res) => {
	const { eventId, eventName, userId, username, club, eventType } = req.body;
	// const participantsdata = {
	// 	// eventId:eventId,
	// 	eventId: eventId,
	// 	eventName: eventName,
	// 	participantsId: userId,
	// 	participantsName: username,
	// 	club: club,
	// 	eventType: eventType,
	// };
	try {
		// Check if the user is already enrolled for the given eventId
		...
		if (existingParticipant) {
			// If the user is already enrolled, respond with a message
			return res
				.status(400)
				.json({ message: "User is already enrolled for this event" });
		}
